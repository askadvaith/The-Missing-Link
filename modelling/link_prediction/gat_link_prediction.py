"""
GAT Link Prediction on Prompt Engineering Knowledge Graph
==========================================================
Uses Graph Attention Networks (GATConv) with multi-head attention
and Dynamic Relative Thresholding for novel technique discovery.

Output: link_pred_output/GAT/run_<timestamp>/
"""
...
class GATLinkPrediction(torch.nn.Module):
    """GAT-based GNN for link prediction with multi-head attention."""
...
def train_model(graph_data, hparams: dict):
    ...
    logging.info(f"Training GAT on {x.shape[1]}-dim semantic features...")
...
def main():
    ...
    run_dir, timestamp = setup_run_directory(model_type="GAT")
    setup_logging(run_dir)

    logging.info(f"Started GAT run in: {run_dir}")
    ...
    with torch.no_grad():
        ...
        if test_pos_edge_index.shape[1] > 0:
            ...
            log_metrics(metrics, prefix="GAT Test")
            save_metrics(metrics, run_dir, model_name="gat")
        else:
            logging.warning("No test edges available for final evaluation.")

    # 7. Discover (Dynamic Relative Thresholding — GAT-specific)
    discover_novel_techniques_relative(
    ...
    # 8. Save model
    model_path = os.path.join(run_dir, "model_gat.pt")
    torch.save(model.state_dict(), model_path)
    logging.info(f"Model saved to {model_path}")
    logging.info("GAT run completed successfully.")
